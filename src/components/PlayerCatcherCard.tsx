...
interface PlayerCardProps {
    ...
    imageUrl: string;
    pcode: string;
    playerData: TPlayerCatcherProps;
}

const PlayerCatcherCard: React.FC<PlayerCardProps> = ({ number, name, imageUrl, pcode, playerData }) => {
  
  const navigate = useNavigate();
  
  const handleClick = () => {
    navigate(`/player/catcher/detail?pcode=${pcode}`, {
      state: { playerData },
    });
  };
...
export default PlayerCatcherCard;
