...
const SeasonTeamRankTable: React.FC<{ recordList: TGameSeasonTeamRecord }> = ({ recordList }) => {
            ...
            <table className="w-full border border-gray-200">
                ...
                <tbody>
                    {table.getRowModel().rows.map((row) => (
                        // KT만 배경색 다르게 처리해야함
                        <tr key={row.id} className={row.getValue("teamName") === "KT" ? "bg-red-100 text-[#EC090C]" : "bg-white"}>
