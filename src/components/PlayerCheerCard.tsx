...
interface PlayerCardProps {
    ...
    imageUrl: string;
    pcode?: string;
    playerData: TPlayerCheerProps;
}

const PlayerCheerCard: React.FC<PlayerCardProps> = ({ number, name, imageUrl, playerData }) => {
      ...
      <div className="absolute inset-0 flex items-center justify-center transition-opacity duration-300 ease-in-out opacity-0 group-hover:opacity-100">
        ...
        >
          ...
          <div className='flex flex-col items-start justify-center space-y-3'>
            <span className="relative z-10 text-[19px] font-semibold text-black">별명: {playerData.leaderNickName} </span>
            <span className="relative z-10 text-[19px] font-semibold text-black">키/혈액형: {playerData.leaderHeight}/{playerData.leaderBloodGroups}</span>
            <span className="relative z-10 text-[19px] font-semibold text-black">직책: {playerData.leaderPosition}</span>
          </div>
        </div>
      </div>
...
export default PlayerCheerCard;
