...
const Infieldersdetail = () => {
  ...
  useEffect(() => {
    const fetchPlayerData = async () => {
      try {
        if (pcode) {
          const data = await api.getPlayerInfielderByPcode(pcode); // pcode로 API 호출하여 개별 선수 데이터 가져오기
  ...
  useEffect(() => {
    const fetchPlayerList = async () => {
      try {
        const data = await api.getPlayerInfielderImage(); // 전체 선수 리스트 API 호출
    ...
    <div className="bg-black min-h-screen w-full">
      ...
      <div className="relative">
        ...
        <div className="absolute top-[422px] w-full px-[144.8px]">
          <TabMenuBar tabs={playerTabs} tabtitle="내야수" />
        </div>
      </div>
      ...
      {isSticky && (
        ...
        >
          <TabMenuNavbar menuItems={playerTabs} tabtitle="내야수" />
        </div>
      )}
      ...
      <div className="relative content_block mt-[40px] px-[144.8px] text-white">
        ...
        {/* 다른 선수 캐러셀 */}
        <div className="w-full mt-8">
          <PlayerCarousel playerList={playerList} position="infielder" />{" "}
...
export default Infieldersdetail;
